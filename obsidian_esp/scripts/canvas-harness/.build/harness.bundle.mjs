...
// obsidian_plugin/src/features/quest-canvas/add-topic.ts
function selectFirstQuestTreeAddTopicTargets(positionedTargets) {
  if (positionedTargets.length === 0) {
    return [];
  }
  const firstX = Math.min(...positionedTargets.map((item) => item.x));
  const firstColumnTargets = positionedTargets.filter((item) => item.x === firstX).sort((left, right) => left.y - right.y || left.nodeId.localeCompare(right.nodeId));
  const bestScore = Math.max(...firstColumnTargets.map((item) => item.score ?? 0));
  const selectedTargets = bestScore > 0 ? firstColumnTargets.filter((item) => (item.score ?? 0) === bestScore) : firstColumnTargets;
  return selectedTargets.map((item) => item.target);
}
...
function hasSelectedQuestJournalFilter(conditions, questIds) {
  return conditions.some((condition) => condition.kind === "journal" && condition.questId !== void 0 && questIds.includes(condition.questId));
}
...
function setCardMeta(node, meta) {
  node.espCard = { ...meta, rev: CARD_META_REV };
}

// obsidian_plugin/src/features/quest-canvas/emit.ts
...
function addFileNode(context, seed, filePath, x, y, width, height, color, espCard) {
...
function addTextNode(context, seed, text, x, y, width, color, espCard) {
  const nodeId = createNodeId(seed);
  if (!context.nodeIds.has(nodeId)) {
    const node = {
      id: nodeId,
...
function addEdge(context, seed, fromNode, fromSide, toNode, toSide, label, derived = false) {
  ...
  context.edges.push({
    ...
    toSide,
    ...label ? { label } : {},
    ...derived ? { espCard: { role: "derived", rev: 1 } } : {}
  });
...
// obsidian_plugin/src/features/quest-canvas/families.ts
...
function resolveAddTopicTransitionTargets(sourceRecord, targetTopic, orderedRecordsByTopic, questIds, context) {
  const targetRecords = orderedRecordsByTopic.get(normalizeTopicKey(targetTopic)) ?? [];
  const candidates = targetRecords.filter((candidate) => conditionsCanFollowAddTopic(sourceRecord, candidate) && (context !== void 0 || questIds === void 0 || recordHasSelectedQuestJournalFilter(candidate, questIds)));
...
function firstQuestTreeAddTopicTargets(context, sourceRecord, candidates) {
  const positionedCandidates = candidates.map((candidate) => {
    const nodeId = context.recordEntryNodeIds.get(candidate.id);
    const node = nodeId ? findCanvasNode(context, nodeId) : void 0;
    return node ? {
      target: candidate,
      nodeId,
      x: node.x,
...
function recordHasSelectedQuestJournalFilter(record, questIds) {
...
// obsidian_plugin/src/features/quest-canvas/discovery.ts
...
function recordLeadsToRelevantRecord(record, topicRecords, recordsByNormalizedTopic, relevant, questIds) {
  return resolveChoiceTargetRecords(record, topicRecords).some((candidate) => relevant.has(candidate.id)) || resolveAddTopicTargetRecords(record, recordsByNormalizedTopic, questIds).some((candidate) => relevant.has(candidate.id));
}
...
function resolveAddTopicTargetRecords(sourceRecord, recordsByNormalizedTopic, questIds) {
  const targetRecords = [];
...
// obsidian_plugin/src/features/quest-canvas/layout.ts
function layoutBranchFamily(context, family, phaseValue, gateX, dialogueX, choiceX, startY, allMilestones, clusterGapY = CLUSTER_GAP_Y) {
  ...
  for (let recordIndex = 0; recordIndex < familyRecords.length; recordIndex += 1) {
    ...
    if (gateId !== dialogueId) {
      addEdge(context, `${gateId}:${dialogueId}`, gateId, "right", dialogueId, "left", void 0, true);
    }
    if (localResultLines.length > 0) {
      ...
      addEdge(context, `${dialogueId}:${resultId}`, dialogueId, "bottom", resultId, "top", void 0, true);
    }
    for (const action of record.resultActions) {
      if (action.kind !== "journal-set" || action.targetJournalIndex === void 0) {
        continue;
...
function choicePromptText(action) {
...
// obsidian_plugin/src/features/quest-canvas/transitions.ts
...
function routeJournalRangeChoiceTransitions(context, records) {
  ...
  for (const [targetNodeId, incomingEdges] of incomingChoiceEdgesByTarget) {
    ...
    for (const item of incomingEdges.sort((left, right) => left.sourcePhase - right.sourcePhase || left.edge.id.localeCompare(right.edge.id))) {
      ...
      addEdge(context, `${jumpNodeId}:${targetNodeId}`, jumpNodeId, "right", targetNodeId, "left", void 0, true);
    }
  }
}
function connectAddTopicTransitions(context, records, questIds) {
...
function connectScriptedMilestones(context, phaseIndices) {
  for (let index = 1; index < phaseIndices.length; index += 1) {
    ...
    addEdge(context, `${fromNode}:${toNode}:scripted`, fromNode, "right", toNode, "left", void 0, true);
  }
}

// obsidian_plugin/src/features/quest-canvas/generate.ts
async function buildQuestCanvas(app2, scope2) {
  ...
  connectAddTopicTransitions(canvasContext, canvasRecords, scope2.questIds);
...
function collectMissingChoiceTargetWarnings(records) {
  const warnings = [];
  const recordsByTopic = /* @__PURE__ */ new Map();
  for (const record of records) {
    const topicRecords = recordsByTopic.get(record.topic) ?? [];
    topicRecords.push(record);
